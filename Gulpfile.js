...
gulp.task('setupDist', function() {
  ...
  return gulp.src(patterns, {base: config.srcDir})
    .pipe($.print())
...
gulp.task('watchForWebpack', function() {
  ...
  watcher.on('change', function(event) {
    ...
    // Run webpack twice.  This is to get around a bug where webpack doesn't
    // notice that the files actually changed.
...
gulp.task('dev', function(cb) {
  if (args.update !== true) {
    // short circuit the update task unless --update is used
    args.noUpdate = true;
  }
...
gulp.task('dev-test', function() {
  ...
  runSequence('prelude', 'clean', 'setupTest', 'webpack:build-dev',
    'watchForWebpack', 'browserSync', 'open', 'serve');
});
...
gulp.task('serve', function(callback) {
  ...
  for (var i=0;i<webpackDevConfig.entry.length;i+=1) {
...
// Default is to run the server task
gulp.task('test', function(cb) {
  ...
  runSequence('prelude', 'update', 'clean', 'setupTest', 'webpack:build-dev', 'watchForWebpack', 'browserSync');
});
...
gulp.task('default', function() {
    ...
    gutil.log("[default]", "------------------------------------------------------");
    gutil.log("[default]", "coverage - executes unit test coverage analysis (TODO)");
    gutil.log("[default]", "dev      - runs project in webpack dev server");
    gutil.log("[default]", "dev-test - runs both tests and dev");
    gutil.log("[default]", "dist     - builds the webpack dist version of project");
    gutil.log("[default]", "test     - runs all tests in browserSync");
    gutil.log("[default]", "------------------------------------------------------");
